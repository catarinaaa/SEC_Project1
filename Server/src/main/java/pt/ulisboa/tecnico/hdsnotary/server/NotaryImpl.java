...
public class NotaryImpl extends UnicastRemoteObject implements NotaryInterface, Serializable {
     ...
     */
    @Override
    public Result intentionToSell(String userId, String goodId, int writeTimeStamp, String cnonce, byte[] writeSignature,
                                  byte[] signature)
    ...
    public Result stateOfGood(String userId, int readID, String cnonce, String goodId,
        ...
        if ((good = goodsList.get(goodId)) != null && cryptoUtils
            ...
            System.out.println("---------------------------\n");
            // TODO change result
            return new Result(good.getUserId(), status, 0, readID, good.getWriterId(), good.getSignature(),
